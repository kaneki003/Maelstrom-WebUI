...
export function TokenPairStats({ poolData }: TokenPairStatsProps) {
  const stats = [
    {
      label: "24h Volume",
      value: `${(Number(poolData.poolStatistics.volume24h.replace(/[^0-9.-]+/g,"")) / 3000).toFixed(2)} ETH`,
      change: `${poolData.liquidityChange24h >= 0 ? "+" : ""}${poolData.liquidityChange24h.toFixed(1)}%`,
      positive: poolData.liquidityChange24h >= 0
    },
    {
      label: "Total Liquidity",
      value: `${(Number(poolData.totalLiquidity.replace(/[^0-9.-]+/g,"")) / 3000).toFixed(2)} ETH`,
      change: `${poolData.liquidityChange24h >= 0 ? "+" : ""}${poolData.liquidityChange24h.toFixed(1)}%`,
      positive: poolData.liquidityChange24h >= 0
    },
    {
  ...
  return (
    <>
      {stats.map((stat) => (
        <Card key={stat.label} className="relative overflow-hidden border-0 group">
          ...
          <CardContent className="relative p-6">
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground/80 font-medium">{stat.label}</p>
